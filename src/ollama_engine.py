        ...
        try:
            ...
            for chunk in self.client.pull(model=model_name, stream=True):
                ...
                if isinstance(completed, int) and isinstance(total, int) and total > 0:
                    ...
                    if status_callback:
                        # Si el callback és el 'print' estàndard, usem \r per sobreescriure la línia
                        if status_callback == print:
                            print(f"\rDOWNLOADING: {percent}%", end="", flush=True)
                        else:
                            status_callback(f"DOWNLOADING: {percent}%")
                elif status:
                    # Per a altres estats, netegem la línia abans d'imprimir (per si venim del % enrere)
                    if status_callback == print:
                        print(f"\r{status.upper()}", end="\n", flush=True)
                    else:
                        status_callback(status.upper())
            
            # Afegim un salt de línia manual en acabar el bucle per no aixafar el READY
            if status_callback == print:
                print() 
