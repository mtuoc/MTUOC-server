...
class Postprocessor:
    def __init__(self, config_dict: dict = None):
        ...
        self.config = {
            ...
            "changes_output_file": None,
            "changes_translation": False,
            "change_stranslation_file": None
        }
        
        if config_dict:
            if "Postprocess" in config_dict:
    ...
    def remove_tags(self, segment: str) -> str:
        ...
        # Patró estricte per a etiquetes XML/HTML: busca qualsevol cosa que comenci 
        # amb '<', opcionalment tingui '/', seguida de caràcters i tanqui amb '>'
        xml_html_pattern = r'</?.+?/?>'
        
        # Eliminem les etiquetes del segment
        clean_segment = re.sub(xml_html_pattern, '', segment)
        
        # Neteja d'espais duplicats marginals que hagin pogut quedar en marxar els tags
    ...
    def align_spacing(self, string1: str, string2: str) -> str:
        ...
        if punc_sequence1 != punc_sequence2:
            print("Atenció: La seqüència de signes de puntuació no coincideix. No es pot realitzar l'ajust.")
            return string2
    ...
    def restore_tags(self,SOURCENOTAGSTOK, SOURCETAGSTOK, SELECTEDALIGNMENT, TARGETNOTAGS, TARGETNOTAGSTOK):
        ...
        for n in range(0,51):
            try:
                ...
                print("MTUOC_Postprocessor restore_tags:",sys.exc_info())
        #finding open tags
        for n in range(0,51):
            try:
                opentag="<tag"+str(n)+">"
                ...
                if len(trobat)>0 and opentag in taglist:
                    ...
                    if not postnum==None and opentag in taglist:
                        try:
                            ...
                            print("ERROR MTUOC_Postprocessor restore_tags:",sys.exc_info())
                        taglist.remove(opentag)
            except:
                print("ERROR MTUOC:Postprocessor restore_tags:",sys.exc_info())
        #finding closing tags
        for n in range(0,51):
            try:
                closingtag="</tag"+str(n)+">"
                regexp=r"[^\s]+ "+closingtag
                trobat=re.findall(regexp, SOURCETAGSTOKNUM, re.DOTALL)
    ...
    def remove_start_end_tag(self, segment):
        ...
        while 1:
            trobat=False
            try:
                starttag=re.match(r"(</?tag[0-9]+>)",segment)
                starttag=starttag.group()
            except:            
                starttag=""
            try:
                endtag=re.search(r"(</?tag[0-9]+>)$",segment)
                endtag=endtag.group()
            except:
                endtag=""
    ...
    def insert_tags(self,SOURCETAGS,TARGETNOTAGS,SELECTEDALIGNMENT,wordaligner):
        try:
            (segmentTAGSMOD,TAGSEQUIL)=self.replace_tags(SOURCETAGS)
            (segmentNOTIF,STARTINGTAG,CLOSINGTAG)=self.remove_start_end_tag(segmentTAGSMOD)
            SOURCENOTAGS=self.remove_tags(SOURCETAGS)
            SOURCENOTAGSTOK=wordaligner.src_tokenizer.tokenize(SOURCENOTAGS)
            SOURCETAGSTOK=wordaligner.src_tokenizer.tokenize(segmentTAGSMOD)
            TARGETNOTAGSTOK=wordaligner.tgt_tokenizer.tokenize(TARGETNOTAGS)
            
            TARGETTAGS=self.restore_tags(SOURCENOTAGSTOK, SOURCETAGSTOK, SELECTEDALIGNMENT, TARGETNOTAGS, TARGETNOTAGSTOK)
            
            if len(STARTINGTAG.strip())>0:
                TARGETTAGS=STARTINGTAG+" "+TARGETTAGS
            if len(CLOSINGTAG.strip())>0:
                TARGETTAGS=TARGETTAGS+" "+CLOSINGTAG
            
            SOURCEEQUILTAGS=SOURCETAGS
            for t in TAGSEQUIL:
                SOURCEEQUILTAGS=SOURCEEQUILTAGS.replace(TAGSEQUIL[t],t,1)
            TARGETTAGS=self.repairSpacesTags(SOURCEEQUILTAGS,TARGETTAGS) #
            for t in TAGSEQUIL:
                try:
                    TARGETTAGS=TARGETTAGS.replace(t,TAGSEQUIL[t],1)
                except:
                    print("ERROR in MTUOC_Postprocessor insert tags:",sys.exc_info())
            return(TARGETTAGS)
                    
            
        except:
            print("Error MTUOC_Postprocessor insert tags",sys.exc_info())
            return(TARGETNOTAGS)
    
    def postprocess(self, translation: dict) -> dict:
        def is_active(key):
            val = self.config.get(key, False)
            if isinstance(val, str):
                return val.strip().lower() in ['true', '1', 'yes', 'on']
            return bool(val)
        if is_active("changes_output") and self.changes_output:
                translation = self.change_output(translation)
        if is_active("changes_translation") and self.changes_translation:
                translation = self.change_translation(translation)
        return translation
    ...
    def change_translation(self, translation):
        for change in self.changes_translation:
            try:
                ...
                tochange=change[2]
                regexpSOURCE=r"\b"+tofindSOURCE+r"\b"
                regexpTARGET=r"\b"+tofindTARGET+r"\b"
