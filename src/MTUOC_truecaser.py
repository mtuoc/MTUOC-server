...
class Truecaser():
    def __init__(self, MTUOCPath=".", tokenizer=None, tc_model=None):
        ...
        if tokenizer==None:
            ...
            # Dynamically load tokenizer using robust file paths
            tokenizer_str = str(tokenizer).strip()
            if not os.path.exists(tokenizer_str):
                raise FileNotFoundError(
                    f"\n[MTUOC TRUECASER ERROR] Tokenizer file not found at: '{tokenizer_str}'\n"
                    f"Please check your Preprocess configuration YAML file."
                )
    ...
    def set_tokenizer(self, tokenizer):
        tokenizer_str = str(tokenizer).strip()
        if not os.path.exists(tokenizer_str):
            raise FileNotFoundError(f"[MTUOC TRUECASER ERROR] Tokenizer not found at: '{tokenizer_str}'")
