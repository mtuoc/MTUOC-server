...
class MTUOCManagerApp:
    ...
    # --- SERVER MANAGEMENT OPERATIONS (ADAPTATS PER A EXECUTABLES) ---
    def start_server(self):
        ...
        try:
            ...
            if getattr(sys, 'frozen', False):
                ...
                cmd = [server_path, config_full_path]
            else:
    ...
    def setup_recipes_tab(self):
        ...
        lbl_list.pack(anchor=tk.W, pady=(0, 5))
        scrollbar = ttk.Scrollbar(list_frame, orient=tk.VERTICAL)
        self.listbox_recipes = tk.Listbox(list_frame, font=("Arial", 10), yscrollcommand=scrollbar.set, selectmode=tk.SINGLE, bg="#ffffff", fg="#000000", highlightbackground="#cccccc")
    ...
    def execute_recipe_download(self, url):
        try:
            ...
            if getattr(sys, 'frozen', False):
                binary_downloader = "MTUOC-downloader.exe" if sys.platform.startswith('win') else "MTUOC-downloader"
                cmd = [os.path.join(self.base_dir, binary_downloader), url]
            else:
                cmd = [sys.executable, "-u", "MTUOC-downloader.py", url]
            ...
            if return_code == 0:
                ...
                self.root.after(0, lambda: messagebox.showerror("Download Error", f"The downloader exited with an error code: {return_code}"))

        except Exception as e:
            self.root.after(0, lambda: messagebox.showerror("Download Error", f"Could not execute downloader:\n{e}"))
        finally:
