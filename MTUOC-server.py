...
if config.MTUOCServer_MTengine=="download":
    engine_repository=configYAML["engine_repository"]
    from HFModelDownloader import HFModelDownloader
    downloader = HFModelDownloader(repo_id=engine_repository)
    download_path = downloader.download_model()
    if download_path:
        test_file_path = os.path.join(downloader.local_dir, "filestomove.txt")
        if os.path.exists(downloader.local_dir):
            downloader.move_files_to_parent_directory()
        else:
            pass
    configfile="config-server.yaml"
...
config.translationMemoryPath=configYAML["TranslationMemory"]["Path"]
config.slMemory=configYAML["TranslationMemory"]["sl"]
config.tlMemory=configYAML["TranslationMemory"]["tl"]


if not config.translationMemoryPath=="None":
    printLOG(1,"LOADING MEMORY",config.translationMemoryPath)
    from MTUOC_memoRetriever import memoRetriever
    config.memoryRetriever = memoRetriever(config.translationMemoryPath)
    config.memoryRetriever.set_languages(config.slMemory, config.tlMemory)
    config.memoryRetriever.disable_fuzzy()
...
if config.AutomaticPostedition=="None": config.AutomaticPostedition=None
config.postedition_sbert_model=configYAML["AutomaticPostedition"]["sbert_model"]
if config.postedition_sbert_model=="None": config.postedition_sbert_model=None
config.postedition_sbert_threshold=float(configYAML["AutomaticPostedition"]["sbert_threshold"])

if not config.AutomaticPostedition==None and not config.postedition_sbert_model==None:
    from sbertScorer import sbertScorer
    calculate_sbert=True
    config.sbertScorer=sbertScorer()
    config.sbertScorer.set_model(config.postedition_sbert_model)
else:
...
if config.MTUOCServer_MTengine=="Marian":
    ...
    config.Marian_prefix=configYAML2["Preprocess"]["prefix"]
    ...
    if config.startMarianServer:
        config.MarianTranslator.start_marian_server()
    config.MarianTranslator.set_prefix(config.Marian_prefix)
    config.MarianTranslator.connect_to_Marian()
    reranking_model=config.sentencepiece=configYAML2["Reranking"]["model"]
    reranking_source_language=config.sentencepiece=configYAML2["Reranking"]["source_language"]
    reranking_source_tokenizer=config.sentencepiece=configYAML2["Reranking"]["source_tokenizer"]
    if reranking_source_tokenizer.startswith("MTUOC"):
        import importlib.util
        if not tokenizer.endswith(".py"): tokenizer=tokenizer+".py"
        spec = importlib.util.spec_from_file_location('', tokenizer)
        tokenizermod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(tokenizermod)
        config.reranktokenizerSL=tokenizermod.Tokenizer()
    
    reranking_glossary_file=config.sentencepiece=configYAML2["Reranking"]["glossary_file"]
    if reranking_glossary_file=="None": reranking_glossary_file=None
    reranking_glossary_separator=config.sentencepiece=configYAML2["Reranking"]["glossary_separator"]
    marian_weight=float(configYAML2["Reranking"]["marian_weight"])
    sbert_weight=float(configYAML2["Reranking"]["sbert_weight"])
    glossary_weight=float(configYAML2["Reranking"]["glossary_weight"])
    neologism_weight=float(configYAML2["Reranking"]["neologism_weight"])
    create_reranker=False
    if sbert_weight==0: reranking_model="None"
    if not reranking_model=="None" and sbert_weight>0: create_reranker=True
    if not reranking_source_language=="None" and neologism_weight>0: create_reranker=True
    if not reranking_glossary_file=="None" and glossary_weight>0: create_reranker=True
    if create_reranker:        
        config.MarianTranslator.set_rescorer(reranking_model,reranking_source_language,reranking_glossary_file,reranking_glossary_separator,marian_weight,sbert_weight,glossary_weight,neologism_weight)
...
if config.MTUOCServer_MTengine=="OpusMT":
    ...
    config.ollama_num_predict=configYAML2["OllamaTranslator"]["num_predict"]
...
elif config.MTUOCServer_MTengine=="GoogleT":
    ###GoogleTranslate imports
    import os
    
    stream2 = open("config-GoogleT.yaml", 'r',encoding="utf-8")
    configYAML2=yaml.load(stream2, Loader=yaml.FullLoader)
    
    Google_sllang=configYAML2["GoogleTranslate"]["sllang"]
    Google_tllang=configYAML2["GoogleTranslate"]["tllang"]
    Google_glossary=configYAML2["GoogleTranslate"]["glossary"]
    if Google_glossary=="None": Google_glossary=None

    #state None if no glossary is used, otherwise the name of the glossary
    Google_project_id=configYAML2["GoogleTranslate"]["project_id"]
    Google_location=configYAML2["GoogleTranslate"]["location"]
    Google_jsonfile=configYAML2["GoogleTranslate"]["jsonfile"]
    os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = Google_jsonfile   
    
    from GoogleTranslateTranslator import GoogleTranslateTranslator
    config.GoogleTranslateTranslator=GoogleTranslateTranslator() 
    config.GoogleTranslateTranslator.set_sllang(Google_sllang)
    config.GoogleTranslateTranslator.set_tllang(Google_tllang)
    config.GoogleTranslateTranslator.set_glossary(Google_glossary)
    config.GoogleTranslateTranslator.set_project_id(Google_project_id)
    config.GoogleTranslateTranslator.set_location(Google_location)
    config.GoogleTranslateTranslator.set_jsonfile(Google_jsonfile)
    ...
    stream2 = open("config-DeepL.yaml", 'r',encoding="utf-8")
    configYAML2=yaml.load(stream2, Loader=yaml.FullLoader)
    
    DeepL_API_key=configYAML2["DeepL"]["API_key"]
    DeepL_sl_lang=configYAML2["DeepL"]["sllang"]
    DeepL_tl_lang=configYAML2["DeepL"]["tllang"]
      #tag_handling: html
      #one of html, xml
    DeepL_formality=configYAML2["DeepL"]["formality"]
      #one of default, less, more
    DeepL_split_sentences=configYAML2["DeepL"]["split_sentences"]
      #one of off, on, nonewlines
    DeepL_glossary=configYAML2["DeepL"]["glossary"]
    if DeepL_glossary=="None": DeepL_glossary=None
    
    config.DeepLTranslator=DeepLTranslator() 
 
    config.DeepLTranslator.set_sllang(DeepL_sl_lang)
    config.DeepLTranslator.set_tllang(DeepL_tl_lang)
    config.DeepLTranslator.set_glossary(DeepL_glossary)
    config.DeepLTranslator.set_formality(DeepL_formality)
    config.DeepLTranslator.set_split_sentences(DeepL_split_sentences)
    config.DeepLTranslator.set_API_key(DeepL_API_key)
