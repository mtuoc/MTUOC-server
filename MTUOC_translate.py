...
def translate_para(paragraph):
    ...
    #paragraph=paragraph.replace("\x10"," ")
    ...
    try:
        if config.segment:
            ...
            for segment in paralist:
                ...
                translation_segment=add_leading_trailing_spaces(translation_segment,lSsegment,tSsegment)
    ...
    printLOG(1,"TRANSLATION PARA:",translation["tgt"])
    return(translation)
...
def translate_string(segment, segment_notags):
    ...
    if config.MTUOCServer_MTengine=="Transformers" or config.MTUOCServer_MTengine=="OpusMT":
        ...
        try:
            ...
            if config.remove_tags:
                translationSTR=config.GoogleTranslateTranslator.translate(segment_notags)
            else:                
                translationSTR=config.GoogleTranslateTranslator.translate(segment)
        except:
            printLOG(1,"Error translating segment Google Translate",sys.exc_info())
            
    elif config.MTUOCServer_MTengine=="DeepL":
        try:
            
            if config.remove_tags:
                translationSTR=config.DeepLTranslator.translate(segment_notags)
            else:                
...
def detect_replace_start_tags(segment):
    pattern = r"^(<.+?>[\sn]*)+"
    match = re.match(pattern, segment)
    if match:
        matched_text = match.group(0)
        remaining_segment = segment[len(matched_text):]
        return match.group(0), remaining_segment
    else:
        return None, segment
        

def detect_replace_end_tags(segment):
    pattern = r"([\sn]*<[^>]+>)+$"
    match = re.search(pattern, segment)
    if match:
        matched_text = match.group(0)
        remaining_segment = segment[:-len(matched_text)]
        return matched_text, remaining_segment
    else:
        return None, segment
...
def translate_segment(segment):
    segment=segment.replace("◄"," ") ###Adhoc EPC
    printLOG(2,"SEGMENT:",segment)
    ...
    if config.preprocessor.has_tags(config.src):
        starting_tags,config.src=detect_replace_start_tags(config.src)
        ending_tags,config.src=detect_replace_start_tags(config.src)
    ...
    match=[]
    foundinTM=False
    if config.memoryRetriever:
        match=config.memoryRetriever.retrieveMTUOC(config.src_notags, min_sim=100, max_results=1)
        config.translation=match
        foundinTM=True
    if not match:
    ...
    if config.sentencepiece and not foundinTM:
    ...
    if config.calculate_sbert:
        try:
            ...
            print("ERRORSBERT",sys.exc_info())
            sbertindex=0
    else:
        sbertindex=0
    sbertindex_float = float(sbertindex) # Asegura que sbertindex sea float
    threshold_float = float(config.postedition_sbert_threshold) # Asegura que el umbral sea float
    
    dopostedition=False
    if sbertindex_float <= threshold_float:
        dopostedition=True
    
    
    if config.AutomaticPostedition=="HFPosteditor" and dopostedition:
    ...
    if config.AutomaticPostedition=="OllamaPosteditor" and dopostedition:
        ...
        config.translation["tgt"]=posteditedTranslation
    addtags=False
    if config.restore_tags and config.preprocessor.has_tags(config.src): addtags=True
    if not starting_tags==None: addtags=True
    if not ending_tags==None: addtags=True
    if addtags:
        try:
            config.postprocessor.insert_tags()
            if not starting_tags==None:
                config.translation["tgt"]=starting_tags+config.translation["tgt"]
            if not ending_tags==None:
                config.translation["tgt"]=config.translation["tgt"]+ending_tags
